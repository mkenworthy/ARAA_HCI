...
if __name__ == "__main__":
	...
	plt.subplot(1,2,1)
	imshow_psf(psf, vmax=1, vmin=1e-6, spatial_resolution=lamD)
	plt.xlabel(r'x ($\lambda / D$)')
	plt.ylabel(r'y ($\lambda / D$)')

	plt.subplot(1,2,2)
	imshow_psf(total_residual, vmax=1e-3, vmin=1e-6, spatial_resolution=lamD)
	plt.xlabel(r'x ($\lambda / D$)')
	plt.ylabel(r'y ($\lambda / D$)')

	plt.savefig('postao_psf_and_coronagraph_image.pdf', bbox_inches='tight')
	plt.show()