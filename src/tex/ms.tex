...
%
With the DM upstream of the WFS, and a control computer providing the calculation of wavefront measured by the WFS and applying this correction to the DM, this forms a {\bf closed loop}, where the response of the DM's correction is seen by the WFS in the instrument, see Figure~\ref{fig:aosystem}.
%
...
\begin{scope}[rotate around={-90:(3.25,14)}]
\draw[domain=3.25:5.75,samples=100,smooth] plot (\x,{0.2*sin(5.44*\x r -2.25 r ) +14});
\end{scope}
\begin{scope}[rotate around={-90:(3.5,14)}]
\draw[domain=3.5:6,samples=100,smooth] plot (\x,{0.2*sin(5.44*\x r -2.5 r ) +14});
\end{scope}
\begin{scope}[rotate around={-90:(3.75,14)}]
\draw[domain=3.75:6.25,samples=100,smooth] plot (\x,{0.2*sin(5.44*\x r -2.75 r ) +14});
\end{scope}
\draw [ rotate around={50:(7.375, 13.25)}] (7,14.75) rectangle (7.75,11.75);
\draw [, rounded corners = 6.6, rotate around={50:(7.125, 9.25)}] (6.75,10.75) rectangle (7.5,7.75);
\draw  (10.75,9.5) ellipse (0.25cm and 1.25cm);
\draw [ fill={rgb,255:red,238; green,255; blue,149} , rotate around={-182:(13.375, 9.75)}] (13,11.25) rectangle (13.75,8.25);
\draw[domain=6:8.25,samples=100,smooth] plot (\x,{0.1*sin(3.01*\x r -5 r ) +11.5});
\draw[domain=6:8.25,samples=100,smooth] plot (\x,{0.1*sin(3.01*\x r -5 r ) +11});
\draw[domain=6:8.25,samples=100,smooth] plot (\x,{0.1*sin(3.01*\x r -5 r ) +11.25});
\draw[domain=6:8.25,samples=100,smooth] plot (\x,{0.1*sin(3.01*\x r -5 r ) +10.75});
\node [font=\large] at (13.25,11.5) {Science Camera};
\node [font=\large, rotate around={-44:(0,0)}] at (6.75,8.75) {Wavefront Sensor};
\node [font=\large, rotate around={-38:(0,0)}] at (9.25,14) {Deformable Mirror};
\end{circuitikz}
}%
 \caption{AO system schematic \textcolor{red}{TODO: label up halos and run for longer}}
  \label{fig:aosystem}
\end{figure}
...
%
Aberrations between this WFS and the final science camera focal plane are referred to Non-Common Path Aberrations (NCPAs).
%
...
%
First generation HC instruments provided little to no active NCPA measurement and mitigation strategies.
%
...
%
As a consequence, subsequent instrument designs have focused on differing mitigation strategies, which include:
\begin{itemize}
